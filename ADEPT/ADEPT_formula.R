...
## for an ADEPT style cluster randomized SMART
## Formulas 5 and 7 in the paper
...
ADEPT_sample_size = function(N = NULL, beta = NULL, alpha, m, delta, ICC, p_1, Cor_Y_X = NULL){
  ...
  if(is.null(N)){
    ...
    cat("The sample size required for an ADEPT design", cov_string, "is", toString(sample_size))
    ...
    cat("The power for an ADEPT design", cov_string, "is", power_val)
...
## Example Run
ADEPT_sample_size(beta = .2, alpha = .05, m = 10, delta = .282, ICC = .01, p_1 = .2, Cor_Y_X = .2)
