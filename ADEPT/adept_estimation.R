...
ADEPT_regression = function(dat, outcome, A1, R, A2, covariates = NULL, cluster) {
	...
	mean_and_V_estimate = function(full_data_frame, r, sigma){
		...
		## the sigma and r for DTR (1,1), (1,-1) and (-1,.), respectively
		...
		for(i in unique(clus_id)){
			...
			if(ind_vec[1]==1 && ind_vec[2] == 1){
				...
				cur_d = cbind(rep(1,cur_m), rep(1,cur_m), rep(1,cur_m), cov_vec)
			...
			## A1 = -1, these clusters are consistent with -1,.
			if(ind_vec[1] == -1){
				
				## We calculate the cluster's contribution makes to A and B due 
				## due to consistency with DTR -1,.	
				...
				## calculate D in estimating equation
				cur_d = cbind(rep(1,cur_m), rep(-1,cur_m), rep(0,cur_m), cov_vec)
		...
		## mean_val is a 3+d length vector containing estimates for
		##  beta_0, beta_1, beta_2, eta_1,...eta_d, respectively.
		mean_val = solve(B, A)
		
		
		## Now we obtain updated for r and sigma based on our mean estimate
		
		## storing residuals now, don't need to actually do
		resid11 = NULL
		resid12 = NULL
		resid2 = NULL
		resid3 = NULL
		...
		## and the denom_var and denom_rho are the denominators.  The 1,2,3
		## correspond, again to DTR (1,1), (1,-1), and (-1,.)
		var_sum1 = 0
		rho_sum1 = 0
		denom_var1 = 0
		denom_rho1 = 0
		...
		for(i in unique(clus_id)){
			...
			if(ind_vec[1]==1 && ind_vec[2] == 1){
					...
					## estimates
					cur_vec = cbind(rep(1,cur_m), rep(1,cur_m), rep(1,cur_m), cov_vec)
			...
			if(ind_vec[1] == -1){
				...
				## Residuals from DTR 1,-1 and the residuals contribution to sigma and r
				## estimates
				cur_vec = cbind(rep(1,cur_m), rep(-1,cur_m), rep(0,cur_m), cov_vec)
				cur_resid = cur_resp - cur_vec%*%mean_val
				inner_p = t(cur_resid)%*%cur_resid
		 		outer_p = cur_resid%*%t(cur_resid)
		...
		return(list(mean_val, c(var_est1, var_est2, var_est3), c(r_est1, r_est2, r_est3), B))
	}
	...
	first_round = mean_and_V_estimate(full_data_frame = full_data_frame, r=c(0,0,0), sigma=c(1,1,1))
	...
	for(i in unique(clus_id)){
		...
		## A1 = -1, these clusters are consistent with -1,. only
		if(ind_vec[1] == -1){
	...
	row.names(mean_val_return) = c("beta_0", "beta_1", "beta_2", eta_names)
	
	row.names(var_cov_mat) = NULL
	var_cov_mat_return = data.frame(var_cov_mat)
	names(var_cov_mat_return) = c("beta_0", "beta_1", "beta_2", eta_names)
	row.names(var_cov_mat_return) = c("beta_0", "beta_1", "beta_2", eta_names)
	
	trans_mat = matrix(c(1,1,1,1,1,-1,1,-1,0), nrow=3, ncol=3)
		
	dtr_mean = data.frame(trans_mat%*%mean_val[1:3,])
	names(dtr_mean) = "DTR mean estimates"
	row.names(dtr_mean) = c("(1,1)", "(1,-1)", "(-1,.)")
	
	dtr_mean_var_cov_mat = data.frame(trans_mat%*%(var_cov_mat[1:3,1:3])%*%(trans_mat))
	names(dtr_mean_var_cov_mat) = c("(1,1)", "(1,-1)", "(-1,.)")
	row.names(dtr_mean_var_cov_mat) = c("(1,1)", "(1,-1)", "(-1,.)")
